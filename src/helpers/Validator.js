...
module.exports = class Validator {
  static validateConfiguration() {
    ...
    if (!process.env.TWITCH_CLIENT_ID || !process.env.TWITCH_CLIENT_SECRET) warn("env: TWITCH_CLIENT_ID or TWITCH_CLIENT_SECRET is missing. Twitch module won't work");
    if (config.MUSIC.ENABLED) {
      if (!process.env.SPOTIFY_CLIENT_ID || !process.env.SPOTIFY_CLIENT_SECRET) {
        warn("env: SPOTIFY_CLIENT_ID or SPOTIFY_CLIENT_SECRET are missing. Spotify music won't work in ErelaJS");
      }
      if (config.MUSIC.NODES.length == 0) {
